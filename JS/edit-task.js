...
function enableDescriptionEdit() {
  ...
  modalDescription.innerHTML = generateDescriptionEditHTML(
...
async function saveTaskChanges() {
  ...
  const column = updateTaskInLocalStorage(updatedTask);
...
function countCompletedSubtasks(subtasks) {
  return subtasks
    .map((_, index) => index)
    .filter(
      (index) => document.getElementById(`subtask-checkbox-${index}`)?.checked,
    );
}
